...
public:
  ...
  map< pair<int,int>, int> &edge;      // arretes du milieu
  bool stereo;
  bool windowdump;
  R rapz;
  int nbiso;
  ...
  R focal;
  int xold,yold;
  Global(Maillage & TTh,map< pair<int,int>, int>& eedge,Rn &ff,int height,int width,R rpz,int nbisovalue,bool st) ;
...
Global::Global(Maillage & TTh,map< pair<int,int>, int>& eedge,Rn &ff,int height,int width,R rpz,int nbisovalue,bool st) 
  : Th(TTh),edge(eedge),stereo(st),f(ff)
{
  nbiso = nbisovalue;
  Width= width ;
  Height=height;
  ...
  for (int i=0;i<Th.nv;i++)
    {
      const  Sommet  & v=Th(i);
      // Les bords du domaine sont bien definis uniquement grace aux valeurs aux sommets
      xmin= min(xmin,v.x);
      ymin= min(ymin,v.y);
      xmax= max(xmax,v.x);
      ymax= max(ymax,v.y);       
	  ...
	  for (int i=0;i<Th.nt;i++)
	    {
	      ...
	      int i0= Th(K[0]), i1= Th(K[1]), i2= Th(K[2]), i3 = Th.numedge(i,edge,0), i4 = Th.numedge(i,edge,1), i5 = Th.numedge(i,edge,2) ;    
	      R2 a0 = K[0], a1 = K[1], a2 = K[2], a3 = .5*(a1+a2), a4 = .5*(a0+a2), a5 = .5*(a0+a1);
	      // On affiche les 4 petits triangles internes
	      {
		R2 Pt[3] = {a0,a5,a4};
		R ff[3] = {f[i0],f[i5],f[i4]};
		DrawIsoTfill(Pt,ff,viso,nbiso,rapz);
	      }
	      {
		R2 Pt[3] = {a5,a1,a3};
		R ff[3] = {f[i5],f[i1],f[i3]};
		DrawIsoTfill(Pt,ff,viso,nbiso,rapz);
	      }
	      {
		R2 Pt[3] = {a4,a3,a2};
		R ff[3] = {f[i4],f[i3],f[i2]};
		DrawIsoTfill(Pt,ff,viso,nbiso,rapz);
	      }
	      {
		R2 Pt[3] = {a3,a5,a4};
		R ff[3] = {f[i3],f[i5],f[i4]};
		DrawIsoTfill(Pt,ff,viso,nbiso,rapz);
	      }
	    }
	}
	  ...
	  for (int i=0;i<Th.nt;i++)
	    {
	      ...
	      DrawVertex(a0,f[i0],rapz);
	      DrawVertex(a5,f[i5],rapz);
	      DrawVertex(a4,f[i4],rapz);
	      glEnd();
	      glBegin(GL_TRIANGLES);
	      DrawVertex(a4,f[i4],rapz);
	      DrawVertex(a5,f[i5],rapz);
	      DrawVertex(a3,f[i3],rapz);
	      glEnd();
	      glBegin(GL_TRIANGLES);
	      DrawVertex(a5,f[i5],rapz);
	      DrawVertex(a1,f[i1],rapz);
	      DrawVertex(a3,f[i3],rapz);
	      glEnd();
	      glBegin(GL_TRIANGLES);
	      DrawVertex(a4,f[i4],rapz);
	      DrawVertex(a3,f[i3],rapz);
	      DrawVertex(a2,f[i2],rapz);
	      glEnd();
	    }
	}
      glEndList();
...
int main(int argc, char** argv)
{
  ...
  global=new Global(Th,edge,f,Height,Width,rapz,nbiso,stereo);
  cout << global->zmax << " " << global->zmin << endl;
  //global->MakeListDraw();    
